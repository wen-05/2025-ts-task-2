                            ...
                            <td class="text-center">
                                <div class="form-check form-switch d-flex justify-content-center align-items-center">
                                    <input style="pointer-events: none" class="form-check-input" type="checkbox"
                                        readonly :checked="Boolean(coupon.is_enabled)" />
                                </div>
                            </td>
                            <td class="text-nowrap">
                                <button @click="openModal(coupon)" class="btn btn-sm btn-outline-dark rounded-lg me-2"
                                    type="button">
                                    編輯
                                </button>
                                <button @click="openDeleteModal(coupon.id)"
                                    class="btn btn-sm btn-outline-danger rounded-lg" type="button">
                                    刪除
                                </button>
                            </td>
                        </tr>
                ...
                <ul class="pagination">
                    <li class="page-item">
                        <button @click="currentPage = String(Number(currentPage) - 1)" :disabled="!pagination?.has_pre"
                            type="button" class="page-link" :class="{ disabled: !pagination?.has_pre }"
                            aria-label="Previous">
                    ...
                    <li class="page-item">
                        <button @click="currentPage = String(Number(currentPage) + 1)" :disabled="pagination?.has_next"
                            class="page-link" :class="{ disabled: pagination?.has_next }" type="button"
                            aria-label="Next">
